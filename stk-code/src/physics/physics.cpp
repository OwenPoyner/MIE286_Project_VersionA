...
void Physics::addKart(const AbstractKart *kart)
{
    ...
    // Disable kart-kart physical collision response at the Bullet level.
    // Karts will still collide with the track and all other objects normally.
    kart->getBody()->setCollisionFlags(
        kart->getBody()->getCollisionFlags() |
...
void Physics::update(int ticks)
{
    ...
    for(p=m_all_collisions.begin(); p!=m_all_collisions.end(); ++p)
    {
        ...
        {
            // Kart-kart collisions are disabled (karts pass through each other).
            // Physical response is suppressed at the Bullet level via
            // CF_NO_CONTACT_RESPONSE in addKart().
            continue;
                             ...
                             btDispatcher* dispatcher)
{
    ...
    for(int i=0; i<currentNumManifolds; i++)
    {
        ...
        else if(upA->is(UserPointer::UP_KART))
        {
            ...
            {
                // 2.2 kart hits kart - disabled, karts pass through each other.
                // Physical response suppressed via CF_NO_CONTACT_RESPONSE in addKart().
            }
            else if(upB->is(UserPointer::UP_PHYSICAL_OBJECT))
            {
