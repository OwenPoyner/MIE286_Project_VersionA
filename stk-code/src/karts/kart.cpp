...
{
    // Group A: Enforce AI karts to be invisible
    if (m_node && m_type != RaceManager::KT_PLAYER)
    {
        if (m_node->isVisible())
            m_node->setVisible(false);
    }
...
void Kart::updateGraphics(float dt)
{
    ...
    if (m_shadow)
    {
        ...
        bool visible = isOnGround() && !emergency;
        // Group A: Hide AI shadows
        if (m_type != RaceManager::KT_PLAYER) visible = false;
        m_shadow->update(visible);
    }
#endif
